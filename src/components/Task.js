...
import { pickRandom } from 'utils/funcs'
...
const Task = ({ context, task, isCreated, isSelected, importedTags }) => {
  ...
  // ---------------- Functions to pass to TagsSelector comp ----------------

  const enterNoMatch = (searchValue) => {
    const color = pickRandom(colorPalatte)
    context.addTag({
      text: searchValue,
      color: color
    })
    context.editTask(task.id, {
      tags: [...task.tags, searchValue]
    })
    context.toasts.success(`Created tag '${searchValue}'`)
  }


  /** If a tag is clicked in the TagsSelector component, add or remove tag from task. */
  const toggleTag = (tagText) => {
    const toggledTag = tags.find((tag) => tag.text === tagText)
    const data = task.tags.includes(toggledTag.text)
      ? task.tags.filter((tags) => tags !== toggledTag.text)
      : [...task.tags, toggledTag.text]
    context.editTask(task.id, {
      tags: data
    })
  }
            ...
            !isCreated || !tags ? null
              : (
                <TagsSelector
                  context={context}
                  tags={tags}
                  bools={tags.map((tag) => task.tags.includes(tag.text))}
                  genOnClick={toggleTag}
                  enterMatched={toggleTag}
                  enterNoMatch={enterNoMatch}
                />
              )
          }
        </div>
